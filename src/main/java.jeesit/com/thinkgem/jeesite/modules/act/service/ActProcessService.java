...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
	...
	/**
	 * 删除部署的流程，级联删除流程实例,级联删除业务表中记录的流程id
	...
	public void deleteProcIns(String procInsId, String deleteReason) {
		//关联清空业务表流程id
		ProcessInstance procIns=runtimeService.createProcessInstanceQuery()
								.processInstanceId(procInsId).singleResult();
		String businessKey=procIns.getBusinessKey();
		if(businessKey!=null && !businessKey.isEmpty()){
			Act act=new Act();
			act.setBusinessTable(businessKey.split(":")[0]);//在启动流程时会将    业务表名+:+业务id  存入businessKey
			act.setProcInsId(procInsId);
			actDao.clearProcInsIdByBusinessId(act);
		}
