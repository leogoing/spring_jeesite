...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		if(act.getTaskDefKey()!=null){//方便查询区分放行和复核
		...
		for (Task task : todoList) {
			result.add(rtnActForTodo(task, "todo"));
		}
		
		// =============== 等待签收的任务  ===============
		getToClaimTask(act, userId, result);
		getToClaimTask(act, "*", result);//包括所有人都可以签收的任务
	 ...
	 * @param result
	 */
	private void getToClaimTask(Act act,String userId,List<Act> result){
		...
		for (Task task : toClaimList) {
			...
			String approveAgain=(String)var.get("approveAgain");
			String recheckAgain=(String)var.get("recheckAgain");
			
			String taskFlag=task.getTaskDefinitionKey();
			if((recheckAgain!=null && "recheck".equals(taskFlag)) || (approveAgain!=null && "approve".equals(taskFlag))){
				taskService.claim(task.getId(), "*".equals(userId)?UserUtils.getUser().getLoginName():userId);
				result.add(rtnActForTodo(task, "todo"));
				continue;
			}
			
			result.add(rtnActForTodo(task, "claim"));
		}
		
	}

	/**根据task内容创建新act对象*/
	private Act rtnActForTodo(Task task,String status){
		Act e = new Act();
		e.setTask(task);
...
//	@Transactional(readOnly = false)
	...
	public void complete(String taskId, String procInsId, String comment, String title, Map<String, Object> vars){
		...
		if(task==null){//针对于贵金属
			task=taskService.createTaskQuery().active().includeProcessVariables().processInstanceId(procInsId).taskDefinitionKey((String)vars.get("taskDefKey")).singleResult();
			taskId=task.getId();
		}
		vars.remove("taskDefKey");
		
		Map<String, Object> varibles=task.getProcessVariables();//TODO  有待改善是否需要把变量重新查出来再存进去
		varibles.putAll(vars);//可能变量传不到后面的任务
		//TODO  可能是activiti的bug已保存在processVariable中的变量在执行到第二个任务后就消失了
		if(varibles.get("recheckGroup")==null){
			varibles.put("recheckGroup", "");
		}
		if(varibles.get("approveGroup")==null){
			varibles.put("approveGroup", "");
		}
		
		String assignee=task.getAssignee();
		if(assignee==null || assignee.isEmpty()){//则表示没有被签收
			return;
		}
		if(task.getDelegationState()!=DelegationState.PENDING){//如果不是委派的任务
			taskService.complete(taskId, varibles);
		}else{
			taskService.resolveTask(taskId, varibles);
		}
