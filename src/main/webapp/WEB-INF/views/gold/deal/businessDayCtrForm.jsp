...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/gold/businessDayCtr/list">营业日管理列表</a></li>
		<li class="active"><a href="${ctx}/gold/businessDayCtr/form?id=${businessDayCtr.id}">营业日管理添加</a></li>
		<shiro:hasPermission name="gold:businessDayDetail:view">
			<li><a href="${ctx}/gold/businessDayCtr/historyList">营业日历史列表</a></li>
		</shiro:hasPermission>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="businessDayCtr" action="${ctx}/gold/businessDayCtr/${flag}" method="post" class="form-horizontal">
		<form:hidden path="id"/> 
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="groupStr" name="groupStr" value="${businessDayCtr.groupStr}" labelName="groupStrNames" labelValue="${businessDayCtr.groupStrNames}"
					title="分组" url="/sys/group/treeData?node=businessDayCtr" cssClass="" notAllowSelectParent="true" checked="true" isGroup="true" node="businessDayCtr"/>
			</div>
		</div>
		<c:if test="${flag!='add'}">
			<div class="control-group">
				...
				<div class="controls">
	                <sys:operateselect id="operate" module="businessDayCtr" objectId="${businessDayCtr.id}"/>
				</div>
			</div>
		</c:if>
		<div class="control-group">
